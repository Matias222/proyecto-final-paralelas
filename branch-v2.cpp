...
void TSPRec(double adj[N][N], int curr_bound, int curr_weight,
...
vector<pair<double,double>> lectura() {
    ...
    std::ifstream inputFile("data_corta.txt");
    std::string line;
...
int main()
{
    ...
    for(int i=0;i<N;i++){
        for(int j=0;j<N;j++){
            ...
            double x1 = lec[i].first, y1 = lec[i].second;
            double x2 = lec[j].first, y2 = lec[j].second;
            adj[i][j]=round(sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2)));
